	...
	try:
		...
		for i in range(len(seedstolook)):
			with h5py.File(infilename) as infile:
				...
				seed = run.attrs["seed"]
				infections = checkIsH5Dataset(run["infections"])[:]
			infections = pd.DataFrame.from_records(infections[infections["inf_loc"] != 2**32-1])
			first_appearences = infections.groupby("mut").first()
			result.append(first_appearences.groupby(["loc","inf_loc"]).count())
		...
		with h5py.File(outfilename) as outfile:
			for groupname in groupnames:
				if groupname in outfile:
					if "geonet_haplos" in checkIsH5Group(outfile[groupname]):
	...
	if nprocs < 2:
		for groupname in tqdm(groupnames, miniters=1, mininterval=1, dynamic_ncols=True):
			...
			if isinstance(result, str):
				writeEmpty(outfilename, groupname, "geonet_haplos", attributes[groupname], result)
			else:
				writeDataset(outfilename, groupname, "geonet_haplos", attributes[groupname], result)
	else:
