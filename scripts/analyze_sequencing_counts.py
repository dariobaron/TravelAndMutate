...
def kernel(tpl):
	try:
		...
		for i in range(len(seedstolook)):
			...
			ids,counts = np.unique(sequencings["id"], return_counts=True)
			df = pd.DataFrame({"id":ids, "counts":counts}, dtype="u4")
		...
		with h5py.File(outfilename) as outfile:
			for groupname in groupnames:
				if groupname in outfile:
					if "sequencing_counts" in checkIsH5Group(outfile[groupname]):
	...
	if nprocs < 2:
		for groupname in tqdm(groupnames, miniters=1, mininterval=1, dynamic_ncols=True):
			...
			if isinstance(result, str):
				writeEmpty(outfilename, groupname, "sequencing_counts", attributes[groupname], result)
			else:
				writeDataset(outfilename, groupname, "sequencing_counts", attributes[groupname], result)
	else:
