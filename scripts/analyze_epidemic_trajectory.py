	...
	try:
		...
		for i in range(len(seedstolook)):
			with h5py.File(infilename) as infile:
				...
				seed = run.attrs["seed"]
				trajectories = [pd.DataFrame.from_records(checkIsH5Dataset(trajectory).fields(["t","I"])[:]) for trajectory in checkIsH5Group(run[f"trajectories"]).values()]
			tot_trajectory = pd.concat(trajectories).groupby("t").sum()
			tot_trajectory["seed"] = np.full(tot_trajectory.shape[0], seed, dtype="u4")
			result.append(tot_trajectory)
		...
		with h5py.File(outfilename) as outfile:
			for groupname in groupnames:
				if groupname in outfile:
					if "epidemic_trajectory" in checkIsH5Group(outfile[groupname]):
	...
	if nprocs < 2:
		for groupname in tqdm(groupnames, miniters=1, mininterval=1, dynamic_ncols=True):
			...
			if isinstance(result, str):
				writeEmpty(outfilename, groupname, "epidemic_trajectory", attributes[groupname], result)
			else:
				writeDataset(outfilename, groupname, "epidemic_trajectory", attributes[groupname], result)
	else:
